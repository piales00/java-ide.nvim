-- java-ide.nvim: crear proyectos Java, clases con su paquete, y compilar / ejecutar /
-- testear desde Neovim como en un IDE.
local M = {}
...
function M.setup(opts)
  ...
  for _, a in ipairs(actions) do
    vim.api.nvim_create_user_command(a[1], M[a[2]], { desc = a[4] })
  end

  local keymaps = config.options.keymaps
  if keymaps then
    for _, a in ipairs(actions) do
      vim.keymap.set("n", keymaps.prefix .. a[3], M[a[2]], { desc = a[4] })
    end
